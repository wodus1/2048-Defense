...
## ▶ 시스템 아키텍처
이 프로젝트는 **2048 퍼즐(보드 시스템)**과 **디펜스 시스템(몬스터·투사체·HP)**을 서로 독립적인 기능 단위로 나누어 구성했습니다.  
각 기능은 `SubSystems`로 모듈화되어 있으며, 공통 관리자는 `SubSystemsManager`가 담당합니다.

- `GameManager`는 **2048 퍼즐 로직과 전체 게임 흐름을 조정하는 중심 관리자** 역할을 합니다.
- 전투·레벨·강화 등 개별 기능들은 `ISubSystem` 인터페이스를 구현하고 `SubSystemsManager`를 통해 초기화 및 접근됩니다.
...
- `layerStatsSystem`의 최종 스탯을 기준으로 사격을 계산합니다
